...
import { Upload, X, Smartphone, Check, Send, MessageCircle, ArrowLeft } from 'lucide-react';
...
export function CustomDesign() {
  ...
  const grandTotal = CUSTOM_PRICE + DELIVERY_FEE;
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4 py-12">
        <div className="bg-white rounded-2xl border border-gray-200 w-full max-w-md overflow-hidden shadow-lg text-center">
          <div className="py-10 px-6 text-white" style={{ background: `linear-gradient(135deg, ${brand.dark}, ${brand.main})` }}>
            ...
            <p style={{ color: brand.light }} className="text-sm">Custom Design Order</p>
          </div>
          <div className="p-6">
            ...
            <div className="bg-gray-50 rounded-xl p-4 text-right mb-5 space-y-1 text-sm" style={{ direction: 'rtl' }}>
              ...
              <p><span className="text-gray-500">السعر:</span> <span className="font-bold" style={{ color: brand.purple }}>{CUSTOM_PRICE} + {DELIVERY_FEE} توصيل = {CUSTOM_PRICE + DELIVERY_FEE} جنيه</span></p>
            </div>
            ...
            <Link to="/products" style={{ color: brand.main }} className="block w-full text-center font-medium py-2 hover:underline">
              العودة للمنتجات
            </Link>
          </div>
        </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Hero */}
      <div className="py-14 px-4 text-white text-center" style={{ background: `linear-gradient(135deg, ${brand.dark}, ${brand.main})` }}>
        <div className="flex items-center justify-center gap-2 mb-4">
          <span className="text-3xl">🎨</span>
        </div>
        <h1 className="text-4xl md:text-5xl font-bold mb-3">Custom Design</h1>
        <p style={{ color: brand.light }} className="text-lg mb-4">صمم كفرك بنفسك — ابعتنا أي صورة وهنطبعهالك</p>
        <div className="inline-block bg-white/20 rounded-full px-5 py-2 font-bold text-lg">
          350 EGP فقط
        </div>
      </div>
        ...
        <div className="space-y-6" style={{ direction: 'rtl' }}>
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-6">
            <h2 className="text-lg font-bold text-gray-900 mb-1 flex items-center gap-2">
              <span className="w-7 h-7 rounded-full flex items-center justify-center text-sm font-bold text-white" style={{ background: brand.main }}>1</span>
              ارفع صورة التصميم
            </h2>
            ...
            {!imagePreview ? (
              <div onClick={() => fileRef.current.click()}
                ...
                style={{ borderColor: dragOver ? brand.purple : errors.image ? '#ef4444' : '#e5e7eb', background: dragOver ? brand.light : 'transparent' }}>
              ...
              <div className="relative">
                <img src={imagePreview} alt="preview" className="w-full max-h-72 object-contain rounded-xl border border-gray-200 bg-gray-50" />
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-6">
            <h2 className="text-lg font-bold text-gray-900 mb-1 flex items-center gap-2">
              <span className="w-7 h-7 rounded-full flex items-center justify-center text-sm font-bold text-white" style={{ background: brand.main }}>2</span>
              وصف التصميم
            </h2>
            ...
            <div className="space-y-4">
              <div>
                ...
                <textarea placeholder="مثال: عايز الصورة على خلفية سودا..." value={form.description}
                  onChange={e => handleChange('description', e.target.value)}
                  rows={4} style={{ ...inp(errors.description), resize: 'vertical' }} />
              ...
              <div>
                ...
                <textarea placeholder="أي تفاصيل تانية..." value={form.notes}
                  ...
                  rows={2} style={{ ...inp(false), resize: 'vertical' }} />
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-6">
            <h2 className="text-lg font-bold text-gray-900 mb-1 flex items-center gap-2">
              <span className="w-7 h-7 rounded-full flex items-center justify-center text-sm font-bold text-white" style={{ background: brand.main }}>3</span>
              موديل التليفون
            </h2>
            ...
            <div className="flex flex-col gap-2 max-h-64 overflow-y-auto pl-1">
              {iphoneModels.map(model => (
                <button key={model} onClick={() => handleChange('model', model)}
                  ...
                  style={{
                    borderColor: form.model === model ? brand.main : '#e5e7eb',
                    background:  form.model === model ? brand.light : '#fff',
                    color:       form.model === model ? brand.main : '#374151',
                  }}>
                  <span>{model}</span>
                  {form.model === model && <Check className="w-4 h-4" style={{ color: brand.main }} />}
                </button>
              ))}
            </div>
            {form.model && <p className="mt-2 text-sm font-medium text-green-600">✓ اخترت: {form.model}</p>}
          </div>

          {/* Step 4: Details */}
          <div className="bg-white rounded-2xl border border-gray-200 p-6">
            <h2 className="text-lg font-bold text-gray-900 mb-1 flex items-center gap-2">
              <span className="w-7 h-7 rounded-full flex items-center justify-center text-sm font-bold text-white" style={{ background: brand.main }}>4</span>
              بيانات التوصيل
            </h2>
            <p className="text-gray-500 text-sm mb-5 mr-9">القاهرة والجيزة فقط</p>
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">المنطقة *</label>
                ...
                {form.governorate && <p className="text-xs mt-1 font-medium" style={{ color: brand.main }}>🚚 رسوم التوصيل: {DELIVERY_FEE} جنيه</p>}
              </div>
              <div>
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-6">
            ...
            <div className="space-y-2 text-sm mb-5">
              <div className="flex justify-between text-gray-600"><span>Custom Design</span><span>{CUSTOM_PRICE} EGP</span></div>
              ...
              <div className="flex justify-between font-bold text-gray-900 text-base border-t pt-2 mt-1">
                <span>الإجمالي</span>
                <span style={{ color: brand.purple }}>{grandTotal} EGP</span>
              </div>
            </div>
            <div className="rounded-xl p-3 mb-5 text-sm" style={{ background: brand.light, direction: 'rtl' }}>
              <p className="font-semibold mb-1" style={{ color: brand.main }}>⚠️ مهم قبل الإرسال</p>
              <p className="text-xs" style={{ color: brand.main }}>بعد تأكيد الطلب — ابعتلنا الصورة على الواتساب وهنبعتلك معاينة التصميم قبل الطباعة.</p>
            </div>
            <button onClick={handleSubmit} disabled={sending}
              ...
              {sending
                ...
                : <><Send className="w-5 h-5" /> تأكيد الطلب — {grandTotal} EGP</>
              }
            </button>
