...
import { Lock, Eye, EyeOff, LogOut, Check, X, Plus, Trash2, ChevronDown, ChevronUp, Package, Upload, AlertTriangle, BarChart2, DollarSign, TrendingUp, ShoppingBag, Tag, Percent } from 'lucide-react';
...
const smallInp = {
  ...
  borderRadius: '6px', fontSize: '12px', color: '#f9fafb',
  outline: 'none', boxSizing: 'border-box', fontFamily: 'inherit',
};
...
export function Admin() {
  ...
  const [newProduct, setNewProduct]   = useState({ name: '', category: 'Anime', description: '', image: '', material: 'Hard Polycarbonate' });
  ...
  useEffect(() => {
    ...
    const unsub = onValue(ref(db, 'orders'), snap => {
      if (snap.exists()) setOrders(Object.values(snap.val()).sort((a, b) => b.timestamp - a.timestamp));
      else setOrders([]);
    });
    return () => unsub();
  }, [authed]);
  ...
  const handleImageUpload = (file) => {
    ...
    reader.onload = (e) => { setNewProduct(p => ({ ...p, image: e.target.result })); setImagePreview(e.target.result); };
  ...
  const handleAddProduct = () => {
    ...
    if (Object.keys(e).length) { setNewProductErr(e); return; }
    addCustomProduct(newProduct);
    setNewProduct({ name: '', category: 'Anime', description: '', image: '', material: 'Hard Polycarbonate' });
    setImagePreview(null); setNewProductErr({}); setShowAddForm(false);
  };
  ...
  const totalCost    = totalItems * Number(costPrice);
  const totalProfit  = totalRevenue - totalCost;
  const deliveryTotal = orders.reduce((s, o) => s + (o.deliveryFee || 0), 0);
    ...
    return (
      <div className="min-h-screen bg-gray-950 flex items-center justify-center px-4">
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-8 w-full max-w-sm text-center">
          ...
          <p className="text-gray-500 text-sm mb-6">Cover Zone</p>
          ...
          <button onClick={handleLogin} className="w-full bg-blue-600 text-white py-3 rounded-xl font-bold hover:bg-blue-700 transition-colors">دخول</button>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <div className="bg-gray-900 border-b border-gray-800 px-4 py-3 flex items-center justify-between sticky top-0 z-10">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 bg-blue-600 rounded-lg flex items-center justify-center"><Package className="w-4 h-4" /></div>
          <div>
        ...
        <button onClick={() => setAuthed(false)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-gray-400 hover:bg-gray-800 text-xs transition-colors">
      ...
      <div className="max-w-4xl mx-auto px-4 pt-6">
        <div className="flex gap-1 mb-6 bg-gray-900 p-1 rounded-xl w-fit flex-wrap">
          {[
            { key: 'stock',   label: '📦 المخزون' },
            { key: 'report',  label: '📊 التقرير' },
            { key: 'orders',  label: '🛍️ الطلبات' },
          ].map(({ key, label }) => (
            <button key={key} onClick={() => setActiveTab(key)}
              className={`px-4 py-2 rounded-lg text-sm font-semibold transition-colors ${activeTab === key ? 'bg-blue-600 text-white' : 'text-gray-400 hover:text-white'}`}>
              {label}
            </button>
          ))}
        </div>
        ...
        <div className="max-w-4xl mx-auto px-4 pb-12 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => setShowAddForm(!showAddForm)}
              ...
              <Plus className="w-4 h-4" /> {showAddForm ? 'إلغاء' : 'إضافة منتج'}
            </button>
          </div>
          ...
          {showAddForm && (
            <div className="bg-gray-900 border border-purple-700/50 rounded-2xl p-5 space-y-4" style={{ direction: 'rtl' }}>
              ...
              <div>
                ...
                {!imagePreview ? (
                  <div onClick={() => fileRef.current.click()}
                    className={`border-2 border-dashed rounded-xl p-6 text-center cursor-pointer ${newProductErr.image ? 'border-red-500' : 'border-gray-700 hover:border-purple-500'}`}>
                  ...
                  <div className="relative">
                    <img src={imagePreview} alt="" className="w-full h-40 object-cover rounded-xl border border-gray-700" />
              ...
              <div>
                ...
                <select value={newProduct.category} onChange={e => setNewProduct(p => ({ ...p, category: e.target.value }))} style={inp(false)}>
                  {categories.filter(c => c !== 'All').map(c => <option key={c} value={c}>{c}</option>)}
                </select>
              </div>
              <div>
                <label className="block text-xs text-gray-400 mb-1.5">الوصف *</label>
                <textarea rows={2} placeholder="وصف المنتج..." value={newProduct.description}
                  onChange={e => setNewProduct(p => ({ ...p, description: e.target.value }))}
                  style={{ ...inp(newProductErr.description), resize: 'vertical' }} />
          ...
          {loading ? (
            ...
            const s        = stock[product.id] ?? { hidden: false, price: 250, salePercent: 0, models: {} };
            ...
            const finalP   = getFinalPrice(product.id);

            return (
              <div key={product.id}
                className={`bg-gray-900 border rounded-xl overflow-hidden ${hidden ? 'border-gray-700 opacity-50' : soldOut ? 'border-red-700/60' : onSale ? 'border-yellow-600/60' : totalQty <= 5 ? 'border-orange-700/60' : 'border-gray-800'}`}>

                {/* Product row */}
                <div className="flex items-center gap-3 p-4">
                  <div className="w-12 h-12 rounded-lg overflow-hidden bg-gray-800 flex-shrink-0">
                    <img src={product.image} alt={product.name} className={`w-full h-full object-cover ${hidden || soldOut ? 'grayscale' : ''}`} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {onSale && <span className="text-xs bg-yellow-900/40 text-yellow-300 px-2 py-0.5 rounded-full">Sale -{s.salePercent}%</span>}
                    </div>
                    <div className="flex items-center gap-2 mt-1 flex-wrap">
                      <span className="text-xs text-gray-500 bg-gray-800 px-2 py-0.5 rounded-full">{product.category}</span>
                      <span className="text-xs text-blue-400">{finalP} EGP {onSale && <span className="text-gray-500 line-through ml-1">{s.price}</span>}</span>
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <button onClick={() => toggleHidden(product.id)}
                      className={`text-xs px-2.5 py-1.5 rounded-lg font-medium transition-colors ${hidden ? 'bg-gray-700 text-gray-300' : 'bg-gray-800 text-gray-400 hover:bg-gray-700'}`}>
                      {hidden ? 'إظهار' : 'إخفاء'}
                    </button>
                    {isCustom && (
                      deleteConfirm === product.id ? (
                        <div className="flex gap-1">
                          <button onClick={() => { deleteCustomProduct(product.id); setDeleteConfirm(null); }}
                            className="text-xs bg-red-600 text-white px-2 py-1.5 rounded-lg">تأكيد</button>
                          <button onClick={() => setDeleteConfirm(null)}
                            className="text-xs bg-gray-700 text-gray-300 px-2 py-1.5 rounded-lg">لا</button>
                        </div>
                      ) : (
                        <button onClick={() => setDeleteConfirm(product.id)}
                          className="text-red-400 bg-gray-800 hover:bg-gray-700 p-1.5 rounded-lg">
                          <Trash2 className="w-4 h-4" />
                        </button>
                      )
                    )}
                    <button onClick={() => setExpanded(isExp ? null : product.id)}
                      className="text-gray-400 bg-gray-800 hover:bg-gray-700 p-1.5 rounded-lg">
                    ...
                    <div className="grid grid-cols-2 gap-3" style={{ direction: 'rtl' }}>
                      <div>
                        <label className="block text-xs text-gray-400 mb-1.5 flex items-center gap-1">
                          <Tag className="w-3 h-3" /> السعر الأصلي (EGP)
                        </label>
                        <input type="number" min="0" value={s.price ?? 250}
                          onChange={e => setProductPrice(product.id, e.target.value)}
                          style={smallInp} />
                      </div>
                      <div>
                        <label className="block text-xs text-gray-400 mb-1.5 flex items-center gap-1">
                          <Percent className="w-3 h-3" /> نسبة الخصم % (0 = مفيش)
                        </label>
                        <input type="number" min="0" max="100" value={s.salePercent ?? 0}
                          onChange={e => setProductSale(product.id, e.target.value)}
                          style={smallInp} />
                      </div>
                    </div>
                    {(s.salePercent > 0) && (
                      <div className="bg-yellow-900/20 border border-yellow-700/40 rounded-lg px-3 py-2 text-xs text-yellow-300" style={{ direction: 'rtl' }}>
                        🏷️ السعر بعد الخصم: <strong>{getFinalPrice(product.id)} EGP</strong> (بدل {s.price} EGP)
                      </div>
                    )}

                    {/* Models */}
                    <div>
                      <p className="text-xs text-gray-500 mb-2" style={{ direction: 'rtl' }}>الكميات لكل موديل — صفر = Sold Out أوتوماتيك</p>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        {iphoneModels.map(model => {
                          ...
                          return (
                            <div key={model}
                              className={`flex items-center justify-between gap-3 px-3 py-2 rounded-lg border ${isSold ? 'bg-red-900/20 border-red-700/40' : qty <= 2 ? 'bg-yellow-900/20 border-yellow-700/40' : 'bg-gray-800 border-gray-700'}`}>
                              <div className="flex items-center gap-2 min-w-0">
                                {isSold ? <X className="w-3 h-3 text-red-400 flex-shrink-0" /> : <Check className="w-3 h-3 text-green-400 flex-shrink-0" />}
                                <span className="text-xs text-gray-300 truncate">{model}</span>
                              </div>
                              <div className="flex items-center gap-1 flex-shrink-0">
                                <button onClick={() => setModelQty(product.id, model, qty - 1)}
                                  className="w-6 h-6 bg-gray-700 hover:bg-gray-600 rounded flex items-center justify-center text-gray-300 font-bold">−</button>
      ...
      {activeTab === 'report' && (
        <div className="max-w-4xl mx-auto px-4 pb-12 space-y-6">
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5" style={{ direction: 'rtl' }}>
            <h3 className="font-semibold text-gray-300 mb-3 text-sm">تكلفة الكفر الواحد</h3>
            <input type="number" value={costPrice} onChange={e => setCostPrice(e.target.value)}
              style={{ ...inp(false), width: '160px', direction: 'ltr' }} />
            <p className="text-xs text-gray-500 mt-1">غيرها حسب تكلفتك الفعلية</p>
          </div>

          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {[
              { icon: <ShoppingBag className="w-5 h-5" />, label: 'إجمالي الطلبات', value: totalOrders,              color: '#3b82f6', bg: '#1e3a5f' },
              { icon: <DollarSign  className="w-5 h-5" />, label: 'إجمالي الإيرادات', value: `${totalRevenue} EGP`, color: '#10b981', bg: '#064e3b' },
              { icon: <TrendingUp  className="w-5 h-5" />, label: 'صافي الربح',       value: `${totalProfit} EGP`,  color: totalProfit >= 0 ? '#f59e0b' : '#ef4444', bg: '#1c1200' },
              { icon: <BarChart2   className="w-5 h-5" />, label: 'إيرادات التوصيل',  value: `${deliveryTotal} EGP`, color: '#8b5cf6', bg: '#2e1065' },
            ].map(({ icon, label, value, color, bg }) => (
              <div key={label} style={{ background: bg, border: `1px solid ${color}30`, borderRadius: 16, padding: 16 }}>
                <div style={{ color, marginBottom: 8 }}>{icon}</div>
                <p style={{ color: '#9ca3af', fontSize: 11, marginBottom: 4 }}>{label}</p>
                <p style={{ color: '#fff', fontSize: 18, fontWeight: 800 }}>{value}</p>
              </div>
            ))}
          </div>
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5" style={{ direction: 'rtl' }}>
            ...
            <div className="space-y-3 text-sm">
              {[
                { label: 'عدد الطلبات',          value: `${totalOrders} طلب`,          color: '#fff' },
                { label: 'إجمالي الإيرادات',     value: `${totalRevenue} EGP`,         color: '#10b981' },
                { label: 'تكلفة المنتجات',       value: `${totalCost} EGP`,            color: '#ef4444' },
                { label: 'إيرادات التوصيل',      value: `${deliveryTotal} EGP`,        color: '#8b5cf6' },
                { label: 'صافي الربح',            value: `${totalProfit} EGP`,          color: totalProfit >= 0 ? '#f59e0b' : '#ef4444' },
          ...
          {orders.length === 0 ? (
            ...
            <div key={i} className="bg-gray-900 border border-gray-800 rounded-xl p-4" style={{ direction: 'rtl' }}>
              <div className="flex items-start justify-between mb-3">
                ...
                <span className="bg-green-900/40 text-green-400 text-xs px-3 py-1 rounded-full font-semibold">{order.grandTotal} EGP</span>
              </div>
              <div className="grid grid-cols-2 gap-2 text-xs text-gray-400">
                <p>👤 {order.name}</p>
                <p>📞 {order.phone}</p>
                <p>📍 {order.governorate}</p>
                <p>🚚 توصيل: {order.deliveryFee} EGP</p>
              </div>
              {order.items && (
                <div className="mt-3 pt-3 border-t border-gray-800">
                  {order.items.map((item, j) => (
                    <p key={j} className="text-xs text-gray-300">• {item.name} ({item.selectedColor}) x{item.quantity} — {item.price} EGP</p>
                  ))}
                </div>
              )}
            </div>
